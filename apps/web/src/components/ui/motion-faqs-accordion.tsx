...
function AccordionItem({
  ...
  return (
    <motion.div
      layout
      className={cn(
        "overflow-hidden rounded-[28px] bg-card text-foreground border border-border",
    ...
    >
      <button
        ...
        onClick={onToggle}
        className="flex w-full cursor-pointer select-none items-center justify-between gap-4 px-6 sm:px-8 py-5 sm:py-6 text-left"
      >
        <span className="text-base sm:text-lg font-medium leading-snug text-foreground">
          {item.question}
        </span>
        ...
        <motion.span
          ...
          className="inline-flex size-10 shrink-0 items-center justify-center rounded-full bg-secondary border border-border text-foreground"
        >
          {isOpen ? (
            <svg
              width="14"
      ...
      >
        <motion.div
          ...
          className="px-6 sm:px-8 pb-6 sm:pb-7"
        >
...
export function MotionAccordion({
  ...
  return (
    <div className={cn("w-full", className)}>
      <div className="flex flex-col rounded-[30px] p-2" style={{ gap }}>
        {items.map((item, i) => (
          <AccordionItem
            key={i}
            item={item}
